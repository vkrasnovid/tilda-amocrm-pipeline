...
class IMAPClient:
    ...
    async def fetch_unseen_senders(self) -> tuple[list[str], list[str]]:
        """
        Search INBOX for UNSEEN messages using UID SEARCH (stable across expunges).
        Returns (sender_emails, uids) — parallel lists.
        """
        _, data = await self._client.uid_search("UNSEEN", charset=None)
        uid_bytes = data[0]
        if not uid_bytes or uid_bytes == b"":
            logger.debug("[imap] UID SEARCH UNSEEN returned 0 message(s)")
            return [], []

        uids = uid_bytes.decode().split()
        logger.debug("[imap] UID SEARCH UNSEEN returned %d message(s)", len(uids))
        ...
        for uid in uids:
            try:
                _, msg_data = await self._client.uid("FETCH", uid, "(BODY[HEADER.FIELDS (FROM)])")
                raw = msg_data[1]
    ...
    async def mark_seen(self, uids: list[str]) -> None:
        """Mark messages with given UIDs as SEEN (UID STORE — stable across expunges)."""
        if not uids:
            return
        uid_list = ",".join(uids)
        await self._client.uid("STORE", uid_list, "+FLAGS", r"(\Seen)")
